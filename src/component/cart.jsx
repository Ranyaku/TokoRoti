...
export default function CartDrop({ cart, total, incItem, deleteItem, role, setRole }) {
  ...
  return (
    <>
      {role === "Admin" && (
        <p onClick={() => navigate("/admin")} className="cursor-pointer px-152 py-1 absolute rounded-md text-sm font-bold transition">
          Dashboard
        </p>
      )}
      ...
      <div className="flex gap-4 text-md items-center relative">
        ...
        <div className="flex gap-4 items-center relative text-sm font-bold">
          ...
          <div className="relative" onMouseEnter={() => setOpen(true)}>
            <button className="relative text-xl" onClick={() => setOpen(!open)}>
              🛒
              <span className="absolute -top-2 -right-3 bg-orange-500 text-white text-xs px-2 rounded-full">{cart.length}</span>
            </button>

            {/* DROPDOWN */}
            {open && (
              <div onMouseLeave={() => setOpen(false)} className="absolute right-0 mt-3 w-72 bg-white border rounded-lg shadow-lg p-4 z-50">
                ...
                <div className="mt-4 border-t pt-3">
                  <button onClick={() => navigate("/checkout")} className="w-full bg-[#242424] text-white py-2 rounded-md hover:opacity-90">
                    Checkout
                  </button>
                </div>
              </div>
